...
local gui = library:New("Jasimware")
...
self:Element("Slider", "distance", {min = 6, max = 40, default = 12}, function(tbl)
	if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
...
movement:Element("Slider", "speed", {min = 15, max = 130, default = 40})
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "buy Eskahook$$$", false, "Innocent", false, true)
